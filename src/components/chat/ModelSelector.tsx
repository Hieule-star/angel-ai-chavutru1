...
export function getModelDisplayInfo(modelId: AIModel) {
  const MODEL_INFO: Record<AIModel, { icon: React.ReactNode; name: string; color: string }> = {
    'google/gemini-2.5-flash-lite-preview': { icon: <Zap className="w-3 h-3" />, name: 'Flash Lite', color: 'text-cyan-500' },
  ...
  return MODEL_INFO[modelId] || MODEL_INFO['google/gemini-2.5-flash-lite-preview'];
}
